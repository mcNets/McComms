using NUnit.Framework;
using System.Net;

namespace McComms.WebSockets.Tests;

[TestFixture]
public class CommsClientWebSocketsTests
{
    [Test]
    public void Constructor_WithDefaultParameters_InitializesClient()
    {
        // Arrange & Act
        var client = new CommsClientWebSockets();

        // Assert
        Assert.That(client, Is.Not.Null);
        // The client should be initialized but not connected at this point
    }

    [Test]
    public void Constructor_WithCustomParameters_InitializesClient()
    {
        // Arrange
        var host = "localhost";
        var port = 8888;

        // Act
        var client = new CommsClientWebSockets(host, port);

        // Assert
        Assert.That(client, Is.Not.Null);
        // The client should be initialized but not connected at this point
    }

    // Note: These tests would need a mock WebSocket server or integration tests with a real server
    // Here we're just testing the initialization of the client classes
}
